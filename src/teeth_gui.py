...
class DentalVisualizationGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("3D Medical Dental Anatomy Visualization System")
        ...
        # Realistic dental anatomy colors
        self.colors = [
            # Tooth enamel (outer layer) - white/ivory
            (0.98, 0.97, 0.94),  # Natural tooth white
            (0.96, 0.95, 0.92),  # Ivory enamel
            (0.97, 0.96, 0.93),  # Pearl white
            (0.95, 0.94, 0.91),  # Light enamel
            (0.99, 0.98, 0.95),  # Bright white
            
            # Dentin (inner tooth layer) - yellowish
            (0.92, 0.88, 0.78),  # Natural dentin
            (0.90, 0.86, 0.76),  # Yellow dentin
            (0.93, 0.89, 0.79),  # Light dentin
            (0.91, 0.87, 0.77),  # Standard dentin
            
            # Tooth pulp (nerve tissue) - pinkish red
            (0.85, 0.35, 0.35),  # Pulp tissue
            (0.88, 0.38, 0.38),  # Vascular pulp
            (0.83, 0.33, 0.33),  # Deep pulp
            
            # Gingiva (gums) - pink
            (0.92, 0.60, 0.65),  # Healthy gum pink
            (0.90, 0.58, 0.63),  # Natural gum tissue
            (0.94, 0.62, 0.67),  # Light gum pink
            (0.88, 0.56, 0.61),  # Darker gum
            (0.91, 0.59, 0.64),  # Standard gum
            
            # Periodontal ligament - pale pink/white
            (0.95, 0.90, 0.88),  # Ligament tissue
            (0.93, 0.88, 0.86),  # Connective tissue
            
            # Alveolar bone (jaw bone) - bone white/cream
            (0.94, 0.91, 0.86),  # Jaw bone
            (0.92, 0.89, 0.84),  # Alveolar bone
            (0.95, 0.92, 0.87),  # Cortical bone
            (0.93, 0.90, 0.85),  # Cancellous bone
            
            # Blood vessels - arteries (red)
            (0.90, 0.15, 0.15),  # Arterial blood
            (0.92, 0.18, 0.18),  # Gingival arteries
            
            # Blood vessels - veins (blue)
            (0.25, 0.35, 0.70),  # Venous blood
            (0.28, 0.38, 0.73),  # Oral veins
            
            # Oral mucosa (mouth lining) - pink/red
            (0.88, 0.52, 0.56),  # Mucosa tissue
            (0.90, 0.54, 0.58),  # Cheek lining
            (0.86, 0.50, 0.54),  # Palate
            
            # Tongue - pink/red
            (0.85, 0.45, 0.50),  # Tongue tissue
            (0.87, 0.47, 0.52),  # Tongue surface
            
            # Cementum (root surface) - yellowish
            (0.93, 0.90, 0.82),  # Cementum layer
            (0.91, 0.88, 0.80),  # Root cementum
        ]
        ...
        self.show_message("🚀 System Ready - Load dental anatomy data to begin visualization")
    
    def get_stylesheet(self):
        """Modern dark theme stylesheet with dental colors"""
        return """
            QMainWindow {
    ...
    def setup_ui(self):
        ...
        title = QLabel("🦷 Dental Anatomy Visualization")
        title.setObjectName("titleLabel")
        header_layout.addWidget(title)
        
        subtitle = QLabel("3D Oral & Maxillofacial Imaging System v2.0")
        ...
        # === LOADED DENTAL PARTS ===
        parts_group = QGroupBox("🦷 Loaded Dental Components")
    ...
    def load_folder_obj(self):
        ...
        folder_path = QFileDialog.getExistingDirectory(
            self,
            "Select Folder with Dental Anatomy Models"
        )
        if not folder_path:
            return
        ...
        try:
            ...
            self.show_message(f"🔄 Loading {len(obj_files)} dental components...")
            ...
            self.data_label.setStyleSheet("color: #58d5ff; font-weight: bold;")
            self.show_message(f"✅ Successfully loaded {loaded_count} dental parts")
    ...
    def load_obj_file(self, file_path):
        ...
        try:
            ...
            if filename in self.dental_parts:
            ...
            # Enhanced realistic dental material properties
            # Enhanced realistic dental material properties - Auto color detection
            filename_lower = filename.lower()

            # اختيار اللون حسب نوع الجزء
            if any(keyword in filename_lower for keyword in ['tooth', 'teeth', 'سن', 'dent', 'incisor', 'molar', 'canine']):
                color = (0.98, 0.97, 0.94)  # أبيض ناصع للأسنان
                print(f"🦷 Tooth detected: {filename}")
            elif any(keyword in filename_lower for keyword in ['gum', 'gingiva', 'لثة', 'tissue']):
                color = (0.92, 0.60, 0.65)  # وردي للثة
                print(f"💗 Gum detected: {filename}")
            else:
                # استخدام الألوان الافتراضية للأجزاء الأخرى
                color = self.colors[self.color_index % len(self.colors)]
                self.color_index += 1
                print(f"🎨 Default color for: {filename}")

            actor.GetProperty().SetColor(color)
            actor.GetProperty().SetSpecular(0.6)  # High specular for tooth shine
            actor.GetProperty().SetSpecularPower(40)  # Sharp highlights
    ...
    def toggle_part_visibility(self, item):
        ...
        if filename and filename in self.dental_parts:
            is_checked = item.checkState() == Qt.Checked
            actor = self.dental_parts[filename]["actor"]
    ...
    def surface_rendering(self):
        """Surface Rendering"""
        if not self.dental_parts:
            self.show_message("⚠️ Please load dental anatomy data first")
            return
        
        for part_data in self.dental_parts.values():
            part_data["actor"].SetVisibility(True)
            part_data["actor"].GetProperty().SetOpacity(0.95)
        
        self.renderer.ResetCamera()
        self.vtk_widget.GetRenderWindow().Render()
        self.show_message(f"🦷 Surface Rendering: {len(self.dental_parts)} components displayed")
        self.info_label.setText(f"🎯 Mode: Surface Rendering | Components: {len(self.dental_parts)}")
    
    def clipping_planes(self):
        """Enable clipping planes"""
        if not self.dental_parts:
            self.show_message("⚠️ Load dental anatomy data first")
            return
        ...
        if not hasattr(self, 'clipping_enabled'):
            ...
            self.info_label.setText("🎯 Mode: Clipping Planes | Slice through dental structure")
        else:
    ...
    def focus_navigation(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("🔍 Focus on Dental Component")
        ...
        # Title
        title = QLabel("Select a dental component to focus on:")
        title.setStyleSheet("font-size: 13px; font-weight: bold; color: #58d5ff;")
        layout.addWidget(title)
        
        # Dropdown with all parts
        combo = QComboBox()
        combo.addItem("-- Select Component --", None)
        for filename in sorted(self.dental_parts.keys()):
    ...
    def apply_focus(self, selected_part, dialog):
        ...
        if not selected_part or selected_part not in self.dental_parts:
        ...
        focused_data = self.dental_parts[selected_part]["data"]
        ...
        for filename, part_data in self.dental_parts.items():
    ...
    def reset_focus(self):
        ...
        self.info_label.setText(f"🎯 Viewport: Rendering {len(self.dental_parts)} components")
        
        if hasattr(self, 'unfocus_btn'):
            self.unfocus_btn.setEnabled(False)
    
    def flythrough_navigation(self):
        """Launch the flythrough navigation window with leg data"""
        if not self.dental_parts:
            self.show_message("⚠️ Please load leg anatomy data first")
            return
        
        try:
            # Import the flythrough window
            from flyThrough2 import FlythroughWindow
            
            # Create and show flythrough window with leg data
            self.flythrough_window = FlythroughWindow(
                system_type="dental",
                parts_data=self.dental_parts  # 🔥 بعت الداتا مباشرة
            )
            self.flythrough_window.show()
            
            self.show_message(f"✈️ Flythrough opened with {len(self.dental_parts)} musculoskeletal components")
            self.info_label.setText(f"🎯 Flythrough: Active with {len(self.dental_parts)} parts")
            
        except Exception as e:
            self.show_message(f"❌ Error launching flythrough: {str(e)}")
            print(f"Error: {e}")
    # ========== CONTROLS ==========
    ...
    def calculate_model_bounds(self):
        ...
        first_part = list(self.dental_parts.values())[0]
    ...
    def create_clipping_controls(self):
        ...
        self.x_clip_checkbox.setStyleSheet("color: #58d5ff; font-weight: bold;")
        ...
        self.clip_x_label.setStyleSheet("background: #21262d; border: 1px solid #30363d; border-radius: 4px; padding: 4px; color: #58d5ff;")
        ...
        self.y_clip_checkbox.setStyleSheet("color: #388bfd; font-weight: bold;")
        ...
        self.clip_y_label.setStyleSheet("background: #21262d; border: 1px solid #30363d; border-radius: 4px; padding: 4px; color: #388bfd;")
    ...
    def update_clipping(self, axis, value):
        ...
        # Calculate bounds of all dental parts to determine appropriate scale
    ...
    def apply_clipping(self):
        """Apply all active clipping planes to all dental parts"""
        if not self.dental_parts:
            return

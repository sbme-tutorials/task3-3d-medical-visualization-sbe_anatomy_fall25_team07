...
class HeartVisualizationGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("3D Medical Heart Anatomy Visualization System")
        ...
        # Realistic heart anatomy colors
        self.colors = self.colors = [
            # Cardiac muscle tissue (myocardium) - deep red/burgundy
            (0.65, 0.16, 0.16),  # Dark red myocardium
            (0.72, 0.18, 0.18),  # Deep cardiac muscle
            (0.68, 0.15, 0.15),  # Rich burgundy muscle
            (0.75, 0.20, 0.20),  # Oxygenated muscle tissue
            (0.70, 0.17, 0.17),  # Standard myocardium
            
            # Arteries - bright red (oxygen-rich blood)
            (0.90, 0.15, 0.15),  # Bright arterial red
            (0.95, 0.20, 0.20),  # Oxygenated arterial blood
            (0.88, 0.12, 0.12),  # Aorta red
            (0.92, 0.18, 0.18),  # Coronary arteries
            
            # Veins - blue/purple (oxygen-poor blood)
            (0.20, 0.30, 0.65),  # Deoxygenated venous blood (blue)
            (0.25, 0.35, 0.70),  # Venous blue
            (0.18, 0.25, 0.60),  # Dark vena cava blue
            (0.30, 0.40, 0.75),  # Pulmonary vein blue
            
            # Cardiac chambers (interior) - darker tones
            (0.60, 0.14, 0.14),  # Ventricle interior
            (0.58, 0.13, 0.13),  # Atrium interior
            
            # Connective tissue, valves, tendons
            (0.92, 0.88, 0.85),  # Valve leaflets (white/cream)
            (0.95, 0.92, 0.88),  # Chordae tendineae
            (0.90, 0.86, 0.82),  # Fibrous tissue
            
            # Pericardium (outer sac) - pale yellow/white
            (0.95, 0.93, 0.88),  # Pericardial sac
            (0.93, 0.90, 0.85),  # Pericardial membrane
            
            # Fat tissue (epicardial fat)
            (0.96, 0.88, 0.60),  # Yellowish epicardial fat
            (0.98, 0.90, 0.65),  # Light cardiac fat
        ]
        # Setup UI
        self.setup_ui()
        ...
        self.show_message("🚀 System Ready - Load heart anatomy data to begin visualization")
    ...
    def setup_ui(self):
        ...
        title = QLabel("❤️ Heart Anatomy Visualization")
        title.setObjectName("titleLabel")
        header_layout.addWidget(title)
        
        subtitle = QLabel("3D Cardiovascular Imaging System v2.0")
        ...
        # === LOADED HEART PARTS ===
        parts_group = QGroupBox("❤️ Loaded Cardiac Components")
        ...
        self.surface_btn = QPushButton("❤️ Surface Rendering")
        ...
        self.movingStuff_btn = QPushButton("⚡ ECG Heart Pumping")
    ...
    def load_folder_obj(self):
        ...
        folder_path = QFileDialog.getExistingDirectory(
            self,
            "Select Folder with Heart Anatomy Models"
        )
        if not folder_path:
            return
        ...
        try:
            ...
            self.show_message(f"🔄 Loading {len(obj_files)} cardiac components...")
            ...
            self.data_label.setStyleSheet("color: #f85149; font-weight: bold;")
            self.show_message(f"✅ Successfully loaded {loaded_count} cardiac parts")
    ...
    def load_obj_file(self, file_path):
        ...
        try:
            ...
            if filename in self.heart_parts:
            ...
            # Enhanced realistic cardiac material properties
            # Enhanced realistic cardiac material properties - Auto detection
            filename_lower = filename.lower()

            # تمييز تلقائي حسب نوع الجزء
            if any(kw in filename_lower for kw in ['ventricle', 'atrium', 'chamber', 'myocardium', 'muscle']):
                color = (0.70, 0.17, 0.17)  # أحمر غامق للعضلة القلبية
                print(f"💪 Cardiac muscle: {filename}")
            elif any(kw in filename_lower for kw in ['artery', 'aorta', 'arterial']):
                color = (0.90, 0.15, 0.15)  # أحمر فاتح للشرايين
                print(f"🔴 Artery: {filename}")
            elif any(kw in filename_lower for kw in ['vein', 'venous', 'vena']):
                color = (0.20, 0.30, 0.65)  # أزرق للأوردة
                print(f"🔵 Vein: {filename}")
            elif any(kw in filename_lower for kw in ['valve', 'leaflet']):
                color = (0.92, 0.88, 0.85)  # أبيض كريمي للصمامات
                print(f"🚪 Valve: {filename}")
            elif any(kw in filename_lower for kw in ['pericardium', 'sac']):
                color = (0.95, 0.93, 0.88)  # بيج للغلاف
                print(f"📦 Pericardium: {filename}")
            else:
                color = self.colors[self.color_index % len(self.colors)]
                self.color_index += 1
                print(f"🎨 Default color: {filename}")
    ...
    def toggle_part_visibility(self, item):
        ...
        if filename and filename in self.heart_parts:
            is_checked = item.checkState() == Qt.Checked
            actor = self.heart_parts[filename]["actor"]
    ...
    def surface_rendering(self):
        ...
        if not self.heart_parts:
            self.show_message("⚠️ Please load heart anatomy data first")
            return
        
        for part_data in self.heart_parts.values():
            part_data["actor"].SetVisibility(True)
            part_data["actor"].GetProperty().SetOpacity(0.95)
        
        self.renderer.ResetCamera()
        self.vtk_widget.GetRenderWindow().Render()
        self.show_message(f"❤️ Surface Rendering: {len(self.heart_parts)} components displayed")
        self.info_label.setText(f"🎯 Mode: Surface Rendering | Components: {len(self.heart_parts)}")
    
    def clipping_planes(self):
        """Enable clipping planes"""
        if not self.heart_parts:
            self.show_message("⚠️ Load heart anatomy data first")
            return
        ...
        if not hasattr(self, 'clipping_enabled'):
            ...
            self.info_label.setText("🎯 Mode: Clipping Planes | Slice through heart structure")
        else:
...
Add this code to yourVisualizationGUI class
"""
    ...
    def focus_navigation(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("🔍 Focus on heart Component")
        ...
        # Title
        title = QLabel("Select a heart component to focus on:")
        ...
        for filename in sorted(self.heart_parts.keys()):
    ...
    def apply_focus(self, selected_part, dialog):
        ...
        if not selected_part or selected_part not in self.heart_parts:
        ...
        focused_data = self.heart_parts[selected_part]["data"]
        ...
        for filename, part_data in self .heart_parts.items():
    ...
    def reset_focus(self):
        ...
        self.info_label.setText(f"🎯 Viewport: Rendering {len(self.heart_parts)} components")
    ...
    def flythrough_navigation(self):
        """Launch the flythrough navigation window with heart data"""
        if not self.heart_parts:
            self.show_message("⚠️ Please load heart anatomy data first")
            return
        ...
        try:
            ...
            # Create and show flythrough window with heart data
            self.flythrough_window = FlythroughWindow(
                system_type="heart",
                parts_data=self.heart_parts  # 🔥 بعت الداتا مباشرة
            )
            self.flythrough_window.show()
            
            self.show_message(f"✈️ Flythrough opened with {len(self.heart_parts)} cardiac components")
            self.info_label.setText(f"🎯 Flythrough: Active with {len(self.heart_parts)} parts")
    ...
    def movingStuff_navigation(self):
        """Launch ECG Heart visualization"""
        try:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            ecg_path = os.path.join(current_dir, "Heart_Moving.py")
            
            if not os.path.exists(ecg_path):
                self.show_message(f"❌ File not found: Heart_Moving.py")
                return
            
            subprocess.Popen([sys.executable, ecg_path])
            self.show_message("✓ Launched ECG Heart Visualization")
    ...
    def calculate_model_bounds(self):
        ...
        first_part = list(self.heart_parts.values())[0]
    ...
    def create_clipping_controls(self):
        ...
        self.x_clip_checkbox.setStyleSheet("color: #f85149; font-weight: bold;")
        ...
        self.clip_x_label.setStyleSheet("background: #21262d; border: 1px solid #30363d; border-radius: 4px; padding: 4px; color: #f85149;")
        ...
        self.y_clip_checkbox.setStyleSheet("color: #ff6b9d; font-weight: bold;")
        ...
        self.clip_y_label.setStyleSheet("background: #21262d; border: 1px solid #30363d; border-radius: 4px; padding: 4px; color: #ff6b9d;")
    ...
    def update_clipping(self, axis, value):
        ...
        # Calculate bounds of all heart parts to determine appropriate scale
    ...
    def apply_clipping(self):
        """Apply all active clipping planes to all heart parts"""
        if not self.heart_parts:
            return
